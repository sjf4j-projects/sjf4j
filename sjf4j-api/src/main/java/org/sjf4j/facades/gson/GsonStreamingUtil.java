...
public class GsonStreamingUtil {
    ...
    public static Object readNode(@NonNull JsonReader reader, Type type) throws IOException {
        ...
        switch (tid) {
            ...
            case FacadeReader.ID_STRING:
                return ConverterRegistry.tryPure2Wrap(nextString(reader), type);
            case FacadeReader.ID_NUMBER:
                return ConverterRegistry.tryPure2Wrap(nextNumber(reader), type);
            case FacadeReader.ID_BOOLEAN:
                return ConverterRegistry.tryPure2Wrap(nextBoolean(reader), type);
    ...
    public static Object readObject(@NonNull JsonReader reader, Type type) throws IOException {
    ...
    @SuppressWarnings("unchecked")
    public static Object readArray(@NonNull JsonReader reader, Type type) throws IOException {
        Class<?> rawClazz = TypeUtil.getRawClass(type);
        NodeConverter<?, ?> converter = ConverterRegistry.getConverter(rawClazz);
    ...
    public static void writeNode(@NonNull JsonWriter writer, Object node) throws IOException {
